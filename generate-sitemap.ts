...
// glossary_terms, leadership, surrey_profiles: static data (not in Supabase) — commented out.
// facts: has title not slug — slug generated from title at runtime.

interface DynamicRouteConfig {
  table: string;
  slugColumn?: string;
  titleColumn?: string; // when set, slug is generated from title
  pathPrefix: string;
  priority: string;
  changefreq: string;
...
async function fetchDynamicRoutes(): Promise<SitemapEntry[]> {
  ...
  for (const config of dynamicRouteConfigs) {
    try {
      const selectCol = config.slugColumn || config.titleColumn!;
      let url = `${SUPABASE_URL}/rest/v1/${config.table}?select=${selectCol}`;
      if (config.filter) {
      ...
      for (const row of rows) {
        const slug = config.slugColumn
          ? row[config.slugColumn]
          : generateSlug(row[config.titleColumn!]);
        if (slug) {
          entries.push({
...
`;
  ...
  console.log(`   ${staticEntries.length} static + ${dynamicEntries.length} dynamic = ${allEntries.length} total URLs\n`);
}
